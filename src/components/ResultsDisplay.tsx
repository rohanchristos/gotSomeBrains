...
const ResultsDisplay = ({ 
  ...
  return (
    <div className="results-container">
      ...
      {/* TensorFlow.js Neural Network Results */}
      {(assessmentData as any).ml_results && (
        <div className="tensorflow-analysis">
          <h3>🧠 TensorFlow.js Neural Network Analysis</h3>
          <div className="neural-network-results">
            <div className="analysis-grid">
              <div className="analysis-item">
                <label>Neural Network Score</label>
                <span className="nn-score">{(assessmentData as any).ml_results.score.toFixed(2)}/100</span>
              </div>
              <div className="analysis-item">
                <label>Risk Level</label>
                <span className={`risk-badge risk-${(assessmentData as any).ml_results.risk_level}`}>
                  {(assessmentData as any).ml_results.risk_level.toUpperCase()}
                </span>
              </div>
              <div className="analysis-item">
                <label>Model Confidence</label>
                <span className="confidence-score">{((assessmentData as any).ml_results.confidence * 100).toFixed(0)}%</span>
              </div>
              <div className="analysis-item">
                <label>Raw Output</label>
                <span className="raw-score">{((assessmentData as any).ml_results.neuralNetworkScore || 0).toFixed(4)}</span>
              </div>
            </div>
            
            {(assessmentData as any).userId && (
              <div className="user-id-info">
                <label>Assessment ID:</label>
                <code>{(assessmentData as any).userId}</code>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="recommendations-section">
        ...
        <div className="recommendations-list">
          {/* Use TensorFlow.js recommendations if available, otherwise use standard ML response */}
          {((assessmentData as any).ml_results?.recommendations || mlResponse.recommendations).map((rec: string, index: number) => (
        ...
        {(assessmentData as any).ml_results && (
          <div className="ml-source-info">
            <small>✨ Recommendations generated by TensorFlow.js neural network</small>
          </div>
        )}
      </div>
